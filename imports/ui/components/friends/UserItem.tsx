...
/*
    const onRemoveFriends = () => {
        props.onRemoveFriend(Meteor.userId(),
            Meteor.user().profile.friends ? Meteor.user().profile.friends.remove([id]) : [id]
        );

        props.onUpdateFriend(
            id,
            user.profile.friends ? user.profile.friends.remove([Meteor.userId()]) : [Meteor.userId()]
        );
    }
*/
    const onMessage = () => {
    ...
    return (
        <StyledUserItem>
            ...
            { isInFriendList() ? (
                <Col>
                    <button className="chat--content-friend">
                        <i className="fas fa-check-circle"></i>
                    </button>
                    <button className="chat--content-send-message" onClick={onMessage}>
                        <i className="far fa-comments"></i>
                    </button>
                    <button className="chat--content-remove-friend" >
