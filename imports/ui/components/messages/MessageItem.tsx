...
const MessageItem = (props: any): JSX.Element => {
    const { picture, lastMessage, onChatClick, _id, active, title } = props;
    ...
    React.useEffect(() => {
        if (active) {
            updateBadges(props.participants, _id);
            badge = getBadges(_id);
        }
    }, [lastMessage])
    console.log(badge,"badge MessageItem");
    ...
    */
    ...
    return (
        <StyledMessageItem active={active}>
            <Avatar large avatar_url={picture} />
            <div className="chat--contentContainer"  onClick={() => onChatClick(_id)}>
