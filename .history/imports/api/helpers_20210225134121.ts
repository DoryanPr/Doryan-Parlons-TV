...
export const getBadges = (chatId: string): number => {
    ...
    console.log(participants,' participants helpers.ts')
    const otherId: string = findOtherId(participants);
    console.log(otherId,' otherId helpers.ts')
    const dd = MessagesCollection;
    const aa = dd.sender.user
    const badge: number = MessagesCollection.find({ chatId, otherId, read: false }).count();
