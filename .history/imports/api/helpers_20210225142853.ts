...
import { Channel, Chat, Message, User } from './models';
...
export const getBadges = (chatId: string): number => {
    ...
    const badge: number = MessagesCollection.find({ chatId, sender: {_id : otherId}, read: false }).count();
    console.log(badge,' badge helpers.ts')

    return badge;
}

export const updateBadges = (participants: string[], chatId: string, _id): void => {
    const otherId: string = findOtherId(participants);
    Meteor.call("message.update.badges", chatId, otherId, _id, (err, res) => {
        if (err) {
            console.log(err, "err")
        } else {
            console.log(res, "res")
