...
export const findChat = (): Chat[] => {
    return ChatsCollection.find().fetch()
        .map(chatCollection => {
            ...
            // const lastMessage: Message = findTwentyLastMessage(chatCollection._id);
            return {
                ...chatCollection,
                title: username,
...
/*
export const findTwentyLastMessage = (): Message => {
    const Msg: Message[] = MessagesCollection.find({
        sort: { createdAt: -1 }
    }).fetch();
    if (!Msg[0]) {
        return ChatsCollection.find().lastMessage;
    }
    return Msg[0];
}
...
export const findOneById = (message): Message[] => {
    const user: User = Meteor.users.findOne({ message.sender._id })
...
export const onChangeUpload = (e: React.ChangeEvent<HTMLInputElement>): any => {
...
export const findTwentyLastMessage = (channel): Message[] => {
    return MessagesCollection.find({ chatId: channel._id }, { sort: { createdAt:  }, limit: 20 }).fetch();
