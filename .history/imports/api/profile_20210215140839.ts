...
export const ProfilCollection = new Mongo.Collection<User>('user');


if (Meteor.isServer) {
    Meteor.publish('Profil.all', function () {
        return Meteor.Profil.find({}, {
            fields: { services: 0 }
        })
    })
    ...
    Meteor.methods({
        "user.insert": function (user) {
            return ProfilCollection.insert(user);
        },
        "user.update": function (_id: string, profile: any, friends: Array<string>) {
            return ProfilCollection.update({ _id }, {
                $set: {
                    $profile: {
                        $set: {
                            friends
...
Meteor.methods({
    ...
    'user.picture': function (imageId: string) {
        ...
        return Meteor.Profil.update({ _id: this.userId }, {
            $set: {
