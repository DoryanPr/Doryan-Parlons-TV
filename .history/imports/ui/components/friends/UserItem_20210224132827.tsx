...
const UserItem = (props: any): JSX.Element => {
    ...
    const onRemoveFriends = () => {
        props.onRemoveFriend(Meteor.userId(),
            Meteor.user().profile.friends ? Meteor.user().profile.friends.splice([id]) : [id]
        );

        props.onUpdateFriend(
            id,
            user.profile.friends ? user.profile.friends.splice([Meteor.userId()]) : [Meteor.userId()]
        );
    }
        ...
        <StyledUserItem>
            ...
            { isInFriendList() ? (
                <Col>
                    ...
                    <button className="chat--content-remove-friend" onClick={onRemoveFriends}>
